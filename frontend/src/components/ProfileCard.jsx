const ProfileCard = ({ isDarkMode, user }) => {
    return (
        <div className="w-full">
            <div className={isDarkMode ? "w-full max-w-sm border py-9 rounded-lg shadow bg-gray-800 border-gray-700" : "w-full max-w-sm bg-white border py-9 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700"}>
                <div className="flex flex-col items-center pb-10">
                    <img className="w-24 h-24 mb-3 rounded-full shadow-lg border-2 border-secondary" src={user && `http://127.0.0.1:8000/storage/usersImages/${user.image}`} alt="Bonnie image"/>
                    <h5 className={isDarkMode ? "mb-1 text-xl font-medium text-white" : "mb-1 text-xl font-medium text-gray-900"}>{user && user.name}</h5>
                    <span className={isDarkMode ? "text-sm text-gray-400" : "text-sm text-gray-500"}>{user && user.email}</span>
