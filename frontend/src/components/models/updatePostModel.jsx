            ...
            <div className="relative flex items-center justify-center w-full h-full max-w-2xl md:h-auto">
                <form method="post" onSubmit={submitForm} className={isDarkMode ? "relative rounded-lg shadow bg-gray-700 w-full" : "relative bg-white rounded-lg shadow w-full"}>
                    ...
                    <div className="p-6 space-y-6">
                        <input ref={titleRef} defaultValue={postDetails.title} className={isDarkMode ? "block p-2.5 w-full text-sm text-white rounded-lg bg-gray-700 border-gray-600 placeholder-gray-400 text-white" : "block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg"} type="text" placeholder="Idea Title..."/>
                        {errors && <p className={isDarkMode ? "text-sm text-red-500" : "text-sm text-red-600"}>{errors.title}</p>}
                        <textarea ref={descriptionRef} defaultValue={postDetails.description} id="message" rows="5"
                    ...
                    <div className={isDarkMode ? "flex items-center p-6 space-x-2 border-t rounded-b border-gray-600" : "flex items-center p-6 space-x-2 border-t rounded-b border-gray-600"}>
                        <button data-modal-hide="staticModal" type="submit" className={isDarkMode ? "text-white focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center bg-blue-600 hover:bg-blue-700 focus:ring-blue-800" : "text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center"}>Update Idea</button>
